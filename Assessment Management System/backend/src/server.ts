...
import { assessmentData, users } from './data/data';
import { assessmentConfigs, getValueFromPath, classifyValue } from './config/config';
...
// Generate PDF Report
app.post('/api/generate-report', authenticateToken, async (req: any, res: any) => {
  try {
    ...
    const assessment = assessmentData.find((item: any) => item.session_id === session_id);
    if (!assessment) {
...
app.get('/api/reports/:filename', (req: any, res: any) => {
...
function generateReportHTML(reportData: ReportData) {
  ...
  const sectionsHTML = reportData.sections.map((section: ReportSection) => `
